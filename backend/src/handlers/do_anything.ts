...
async function takeScreenshot(): Promise<{ filePath: string; method: string }>{
  ...
  throw BAD_REQUEST('No screenshot tool available (need scrot, gnome-screenshot, or ImageMagick import)');
}
...
export async function handleDoAnything(args: unknown, context: LogContext) {
  ...
  for (let step = 1; step <= maxSteps; step++) {
    ...
    if (!dryRun) {
      ...
      transcript[transcript.length - 1].execution = execResult;
    }
  ...
  return {
    content: [
      { type: 'json', data: { goal, steps: maxSteps, done: false, reason: 'max_steps', transcript: transcript } }
    ]
  };
}