...
export function resolveSafePath(inputPath: string, cwd?: string): { abs: string; root: string } {
  ...
  const root = roots.find((r) => isSubPathOf(real, r));
  if (!root) {
...
export function guessIsText(filename: string): boolean {
  const textExts = new Set([
    '.txt','.md','.json','.yaml','.yml','.xml','.csv','.log','.gitignore','.env',
    '.ts','.tsx','.js','.jsx','.mjs','.cjs','.css','.scss','.sass','.less','.html','.htm',
    '.py','.rb','.rs','.go','.java','.kt','.swift','.c','.cc','.cpp','.h','.hpp','.sql','.sh','.bash','.zsh',
  ]);
...
export function listDirSafe(abs: string, opts: { includeHidden?: boolean } = {}) {
  ...
  const items = entries
    ...
    .map((d) => {
      ...
      const type: 'file' | 'dir' | 'symlink' = d.isDirectory() ? 'dir' : d.isSymbolicLink() ? 'symlink' : 'file';
      let size = 0;
      let mtimeMs = 0;
      try {
...
export function walkFilesSafe(absDir: string, opts: { includeHidden?: boolean; maxFiles?: number; ignore?: (p: string)=>boolean } = {}) {
